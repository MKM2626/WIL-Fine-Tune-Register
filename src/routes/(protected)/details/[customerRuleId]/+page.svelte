<script lang="ts">
    import { deleteFineTune } from "#lib/remote/delete.remote";
    ...
    let ftSearchInfo = $state({ customerRuleId: "", page: 1, pageSize: 50, descending: true })

    $effect(() => {
        ftSearchInfo.customerRuleId = id
    })
    // const moveOut = 0;
    // const delay = 0;
    // const moveIn = 0;
    ...
    // let selectedFineTune = $derived(await getFineTuneDetails(ftSearchInfo))
    ...
    async function del() {
        ...
        try {
            await deleteFineTune(id).updates(
                getCustomerRules(searchInfo).withOverride((results) => ({
                    ...results
                }))
            );
