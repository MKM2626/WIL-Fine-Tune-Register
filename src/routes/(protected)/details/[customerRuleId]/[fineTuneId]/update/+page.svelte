<script lang="ts">
    ...
    import { updateForm } from "#lib/remote/updateForm.remote";
    ...
    import { Globe, CircleCheck, CircleX, CalendarDays } from '@lucide/svelte/icons'
    ...
    let crId = $derived(params.customerRuleId)
    ...
    onMount(() => {
        updateForm.fields.fineTune.set(fineTune.after)
        if (fineTune.comment) updateForm.fields.comment.set(fineTune.comment)
        if (fineTune.name) updateForm.fields.name.set(fineTune.name)
        if (fineTune.tags) updateForm.fields.existingTags.set(fineTune.tags.map((tag) => tag.id))
        updateForm.fields.newTags.set([])
        if (!fineTune.finalised) history.back()
        updateForm.fields.customerRuleId.set(crId)
    })

</script>

<form class="max-w-5xl" {...updateForm.enhance(async (form) => {
    try {
    ...
    <input {...updateForm.fields.customerRuleId.as('select')} type='hidden'/>
            ...
            {#if !atLeast($session.data?.user.role, 'senior')}
                <div class="mt-3 grid grid-cols-1 gap-x-6 gap-y-3 border-t border-border pt-3 @md:grid-cols-3">
                    <div>
                        ...
                        <p class="wrap-break-words text-sm text-text">False</p>
                    </div>
            ...
            <div class="border-t border-border px-5 py-4">
                <label class="block">
                    ...
                    <textarea
                        rows="6"
                        use:tabIndentation
                        {...updateForm.fields.fineTune.as('text')}
                    ...
                    ></textarea>
                    {#each updateForm.fields.fineTune.issues() as issue (issue)}
        ...
        <section class="rounded-xl border border-border bg-bg-light shadow-md">
            <div class="px-5 py-4">
                <label class='block'>
                    ...
                    <input 
                        {...updateForm.fields.expiryDate.as('date')}
            ...
            <div class="border-t border-border px-5 py-4">
                <label class="block">
                    ...
                    <textarea
                        ...
                        {...updateForm.fields.comment.as('text')}
            ...
            <div class="border-t border-border px-5 py-4">
                ...
                <TagInput
                    ...
                    onChange={(existing, created) => {
                        updateForm.fields.existingTags.set(existing)
                        updateForm.fields.newTags.set(created)
                    }}
                />
            </div>
        </section>
        ...
        {#if atLeast($session.data?.user.role, 'senior')}
            <section class="rounded-xl border border-border bg-bg-light shadow-md">
                <div class="grid grid-cols-1 gap-x-6 gap-y-4 px-5 py-4 @md:grid-cols-2">
                    ...
                    <input {...updateForm.fields.global.as('hidden', global)}>
                    ...
                    >
                        <span class=" items-center font-medium tracking-wide text-text inline-flex gap-x-2">
                            {#if updateForm.fields.finalised.value()}
                    ...
                    <input {...updateForm.fields.finalised.as('hidden', finalised)}>
