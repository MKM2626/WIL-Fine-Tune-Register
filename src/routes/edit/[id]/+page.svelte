<script lang="ts">
	...
	import { boolean } from "drizzle-orm/pg-core/columns";
...
<form {...editForm} class="max-w-5xl">
    ...
    <input type="hidden" {...editForm.fields.id.as("select", id)}/>
    ...
    <div class="bg-bg-light border border-border rounded-lg p-5 flex flex-col gap-3">
        ...
        {#if fineTune.finalised} 
            <div class="flex">
                ...
                <input type="checkbox" checked={fineTune.finalised} disabled 
                    class={`appearance-none h-3 w-3 border rounded-xs shadow-sm align-bottom -bottom-0.75 relative flex items-center justify-center font-bold text-[12px] text-white cursor-pointer
                        ${fineTune.finalised ? 'bg-green-500 border-green-500 before:content-["✓"]' : 'bg-red-500 border-red-500 before:content-["✗"]'}`}
                >
            </div>
        {/if}
        ...
        <div class="flex">
            ...
            <input type="checkbox" checked={fineTune.global} disabled class={`appearance-none h-3 w-3 border rounded-xs shadow-sm align-bottom relative -bottom-0.75 flex items-center justify-center font-bold text-[12px] text-white cursor-pointer
                ${fineTune.global ? 'bg-green-500 border-green-500 before:content-["✓"]' : 'bg-red-500 border-red-500 before:content-["✗"]'}`}
            >
        </div>
    </div>
    ...
    {#if !fineTune.finalised}
        <div class="mt-5">
            ...
            <div class="bg-bg-light border border-border rounded-lg p-4 flex items-center gap-3">
                ...
                <input
                    {...editForm.fields.finalised.as("checkbox", false)}
                    class={`appearance-none h-3 w-3 border rounded-xs shadow-sm align-bottom relative flex items-center justify-center font-bold text-[12px] text-white cursor-pointer
                        ${editForm.fields.finalised.value() ? 'bg-green-500 border-green-500 before:content-["✓"]' : 'bg-red-500 border-red-500 before:content-["✗"]'}`}
                />  
                <label for='finalise'>
    ...
    <div class="mt-5">
        ...
        <div class="bg-bg-light border border-border rounded-lg p-4">
           <select
                ...
                {...editForm.fields.analystId.as("select", fineTune.analystId!)}
            >
